        ...
        <ul class="">
          <li v-for="(item,index) in dataArr" :key="item.title">
            ...
            <div class="tc titleDiv">
              <span :class="{'title':true,'titleSmall':false,'flashlight':true}">{{item.title}}</span>
            </div>
            <div class="tc singerDiv">
              <span class="singer">{{item.type}}</span>
              <span class="singer">{{item.type}}</span>
            </div>
            <div class="tc trendDiv">
              <span class="yellow titleBigger">{{item.grade}}</span>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    initData(){
      let json = require("@/assets/json/TadGames/M8HotPlayGames.json")
      this.dataArr = json
...
<style>
  ...
  /*整体左移*/
  .mainstay ul{
    animation: leftShift calc(2.7*21s) linear;
